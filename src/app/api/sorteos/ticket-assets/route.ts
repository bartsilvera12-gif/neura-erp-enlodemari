...
/**
 * POST multipart: sorteo_id, kind=logo|background, file
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (kind !== "logo" && kind !== "background") {
      return NextResponse.json(errorResponse("kind debe ser logo o background"), { status: 400 });
    }
...
/**
 * DELETE ?sorteo_id=&kind=logo|background
